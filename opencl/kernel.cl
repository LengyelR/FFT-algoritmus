...
// elkészítjük a Twiddle factorokat a későbbi számításokhoz
__kernel void spinFact(__global float2* w, int n)	
{	
	unsigned int i = get_global_id(0);	

	//PI_2 segítségével a -sin() SIMD műveletekkel számolhatjuk
	float2 angle = (float2)(2*i*PI/(float)n,(2*i*PI/(float)n)+PI_2);	
	w[i] = cos(angle);	
}	
	
//bitműveletekkel a standart hatékony reverzió
...
// inputot normáljuk, azaz elosztjuk n-el
